    ...
    if request.method == 'POST':
        ...
        if cart:
            ...
            if basket:
                ...
                basket.save()
                messages.success(request,format_html('one item added to cart ... <a href="http://54.242.244.61/cart"> view cart</a>'))
                return redirect('home')
            else:
                newitem = Cart()
...
def pay(request):
    if request.method == 'POST':
        ...
        cburl = 'http://54.242.244.61/callback' #payment confirmation page 
